...
class ProcessData:
	...
	def load_train_data(self,path):
		...
		print("There are %-3d classes in this dataset" % len(type_label))
		print("There are %-3d samples in this dataset" % len(labels))
		...
		path_dir,name_file=os.path.split(path)
		path_save=os.path.join(path_dir,'intent_label_relation.csv')
	...
	def split_dataset(self, data, labels):
		...
		train_x, val_x, train_y, val_y = train_test_split(data,labels,test_size = 0.2,stratify= labels)
	...
	def prepare_data_loader(self, path_data, path_save_split,mode_test=False):
		'''
		Prepare dataloader for training, validation or test
		'''
		if mode_test:
			# load dataset
	...
	def train_model(self,train,validation):
		...
		#Evaluate model on validation set after each training epoch
			...
			print(f'Accuracy:%.4f; Precision:%.4f; Recall:%.4f; F1_score:%.4f;' % (avg_val_accuracy,
																				   ...
																				   avg_val_recall,
																				   avg_val_f1))
			print(f'[Whole set]Accuracy:%.4f; Precision:%.4f; Recall:%.4f; F1_score:%.4f;' % (acc,
			                                                                       pre,
																				   rec,
																				   f1))
			print('\n')
...
if __name__ == '__main__':
	...
	parser.add_argument('--name_dataset', default="annotated_dataset.csv")
	...
	parser.add_argument('--data_prepare_mode', default=False) # False means prepare data for training 
	...
	parser.add_argument('--batch_size_tes', type=int, default=64) # inference batch size
	...
	# model directory:https://huggingface.co/transformers/pretrained_models.html
	# pretrained model can be downloaded here:https://huggingface.co/bert-base-uncased/tree/main

    # set the dataset path
	path_train_data = os.path.join(args.path_project_folder, args.name_dataset)

	# set save model path
	path_checkpoint_folder = os.path.join(args.path_project_folder, "checkpoint")
	os.makedirs(path_checkpoint_folder, exist_ok=True)
	path_saved = os.path.join(path_checkpoint_folder, args.pretrained_model_name)

	# initialize the data processor
	processor = ProcessData(tokenizer_vocab_path = args.pretrained_model_name,
				            tokenizer_max_len = args.token_max_len,
							feature_name = args.data_feature_type # choose traslated text or original text 
							)
	print('[step-1] The data processor is initialized')
	...
	print('[step-2] The model is initialized')

	# prepare for model dataloader
	train,validation = processor.prepare_data_loader(path_train_data,
													path_save_split= args.path_project_folder,
	                                                mode_test=args.data_prepare_mode
													)
	print('[step-3] The model dataloader is ready')
    
	# start training the model based on our dataset
	My_XLMRobertaModel.train_model(train,validation)

	print('The whole training procedure spends %.3f minuts'%((time.time()-start_time)/60))